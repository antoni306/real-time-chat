...
export class AuthService {
    ...
    constructor(@InjectRepository(User) private readonly userRepo:Repository<User>,private readonly configService:ConfigService,private readonly jwtService:JwtService){}
    async register(registerDto:RegisterDto):Promise<void>{
        const user = await this.userRepo.findOneBy({username:registerDto.username});
        
        if(user){
            throw new ConflictException("this username is already taken");
        }
        const saltRounds = parseInt(this.configService.get<string>("SALT_ROUNDS") as string);

        const hash = await bcrypt.hash(registerDto.password,saltRounds);
        await this.userRepo.insert({username:registerDto.username,email:registerDto.email,passwordHash:hash});
    }
